module.exports = (client) => {
    ...
    // Rank table
    const RANKS = {
        PE: { name: "Premium Economy", price: 199, prefix: "[PE]", color: 0xfd6727 },
        BC: { name: "Business Class", price: 499, prefix: "[BC]", color: 0xa6a6a6 },
        FC: { name: "First Class", price: 799, prefix: "[FC]", color: 0xa6a6a6 },
        FLM: { name: "Phoenix Flames", price: 1399, prefix: "[FLM]", color: 0xcda434 }
    };
    ...
    app.post("/purchase", async (req, res) => {
        ...
        if (!RANKS[requestedRankKey]) return res.status(400).json({ error: "Invalid rank" });
        ...
        try {
            const channelReq = await client.channels.fetch(CHANNEL_RANK_REQUEST);
